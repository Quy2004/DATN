...
const DetailPage = () => {
    ...
    const formatPrice = (basePrice: number, sizePrice: number, quantity: number) => {
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="mb-4">
                                <span className="text-lg font-medium mr-2">
                                    {formatPrice(product.price, selectedSize?.size_id.priceSize || 0, quantity)} VNĐ
                                </span>
                            </div>
                            <hr className="mb-3" />
                            <div className="mb-6">
                                        ...
                                        selectedProduct.product_sizes.length > 0 ? (
                                            ...
                                            ${selectedSize?.size_id._id === size.size_id._id
                                                    ...
                                                    >
                                                        ...
                                                        <span className="ml-2 text-sm">+ {listPrice(size.size_id.priceSize || 0)} VNĐ</span>
                                                    </button>
                                                ))}
                                            </div>
                                        ) : (
                                            <p className="px-6 text-gray-500">Không có kích thước nào có sẵn.</p>
                                        )
                                    ) : (
                                        <p className="px-6 text-gray-500">Vui lòng chọn một sản phẩm để xem kích thước.</p>
                                    )}
                                        ...
                                        {selectedProduct ? (
                                            <form className="flex flex-col my-1 rounded-md">
                                                {selectedProduct.product_toppings.map((topping) => {
                                                    return (
                                                        <div key={topping.topping_id._id} className="flex items-center gap-2 px-2 py-2 border-b border-gray-200">
                                                            <input
                                                                type="checkbox"
                                                                name="topping"
                                                            ...
                                                            <label className="flex-1">
                                                                ...
                                                                {topping.topping_id.priceTopping ? ` ( + ${listPrice(topping.topping_id.priceTopping)} VNĐ)` : null}
                                                            </label>
                                                        </div>
                                                    );
                                                })}
                            ...
                            {products.slice(0, 7).map((item) => (
                                item._id !== product._id ? (
                                    <div key={item._id} className="flex flex-col items-center">
                                        ...
                                        <Link to={`/detail/${item._id}`}>
                                            <h3 className="mx-2 text-center">{item.name}</h3>
                                        </Link>
                                        <p className="mx-2 text-xs text-[#838080] text-center">
