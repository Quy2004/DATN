...
const MenuPage: React.FC = () => {
  ...
  // Cập nhật useQuery cho sản phẩm
  const { data: products, isLoading: loadingProducts } = useQuery({
    queryKey: ["products", activeItem, currentPage],
    queryFn: async () => {
      let url = "/products";
      if (activeItem) {
  ...
  const handleClick = async (id: string, name: string) => {
    ...
    if (childCategories && childCategories.length > 0) {
      ...
      // Nếu không có danh mục con, có thể lấy sản phẩm của danh mục cha
          ...
          {parentCategories.map((parent: Category) => (
            <li key={parent._id} className="mb-1">
              ...
              <div
                ...
                className={`cursor-pointer flex items-center justify-between py-2 px-3 rounded-md transition-all duration-300 ease-in-out ${activeItem === parent._id
                    ? "font-bold text-orange-600 bg-orange-50"
                    : "text-gray-700 hover:text-orange-600 hover:bg-orange-50"
                  }`}
              >
