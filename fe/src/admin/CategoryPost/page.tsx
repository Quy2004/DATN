	...
	const mutationSoftDelete = useMutation<void, Error, string>({
		mutationFn: async (_id: string) => {
			try {
				...
				throw new Error("Xóa mềm danh mục bài viết thất bại");
			}
		},
		onSuccess: () => {
			messageApi.success("Xóa mềm danh mục bài viết thành công");
	...
	const mutationHardDelete = useMutation<void, Error, string>({
		mutationFn: async (_id: string) => {
			try {
				...
				throw new Error("Xóa cứng danh mục bài viết thất bại");
			}
		},
		onSuccess: () => {
			messageApi.success("Xóa cứng danh mục bài viết thành công");
			queryClient.invalidateQueries({ queryKey: ["categoryPost"] });
		},
		onError: error => {
			messageApi.error(`Lỗi: ${error.message}`);
			...
			render: (_: string, categoryPost: CategoryPost) => (
				<Space size="middle">
					{isDelete ? (
						<>
							<Popconfirm
								title="Khôi phục danh mục bài viết"
								description="Bạn có chắc chắn muốn khôi phục danh mục bài viết này?"
							...
							<Popconfirm
								title="Xóa cứng danh mục bài viết"
								description="Bạn có chắc chắn muốn xóa danh mục bài viết này vĩnh viễn?"
						...
						<>
							<Popconfirm
								title="Xóa mềm danh mục bài viết"
								description="Bạn có chắc chắn muốn xóa mềm danh mục bài viết này?"
	...
	return (
		<div>
			<div className="flex items-center justify-between mb-5">
				<Title level={3}>Danh sách danh mục bài viết</Title>
				{contextHolder}
